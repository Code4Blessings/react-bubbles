...
- [X] Explain what a token is used for.
    ...
    --Token are strings of cryptic text issued by the server.  The token will then enable the server to tell if the client has authorization to receive a data transfer.

- [X] What steps can you take in your web apps to keep your data secure?

    1.	Keep your web app up to date.
    2.	Create login page along with a protected page (embedded in a private route) and password.
    3.	Have the server issue a token encrypts the data.  When the client makes a request for data, using a username and password, the server can read the token and decide if the client has permission to receive the data transfer.
...
- [X] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.
    ...
    --Create, UPDATE, and Delete which is axios.create and axios.put, and axios.delete, respectively.
